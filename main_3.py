...
class Job_first:
    ...
    def job(self):
        print("okk")
        query = f"SELECT * FROM Send WHERE categoryId=3 AND id={self._id}"
    ...
    def execute_cron_jobs(self):
        try:
            ...
            query = f"SELECT *  FROM Send WHERE  interval NOTNULL AND strftime('%H:%M', interval) >= '{_time}' AND categoryId=3 ORDER BY  strftime('%H:%M', interval) ASC"
            print(query)
            data = DBS.post_sql_query(query)
